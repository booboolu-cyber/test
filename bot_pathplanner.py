...
# -*- coding: utf-8 -*-
"""
Path Planning Module for Bot Navigation
---------------------------------------
Purpose: Module to perform path planning from source to destination using provided methods.
         [DFS, DFS_Shortest, Dijkstra, A*]

Usage: You can perform path planning by
...
3) Maze Image
4) Method to Use [DFS, DFS_Shortest, Dijkstra, A*]
...
Author: Based on Haider Abbasi's original code
Date: 2024
"""
...
class bot_pathplanner():
    ...
    def find_path_nd_display(self, graph, start, end, maze, method="DFS"):
        ...
        if (method == "dijkstra"):
            if (self.dijkstra.shortest_path_overlayed == []):
                self.draw_path_on_maze(maze, pathpts_to_display, method)
            else:
                cv2.imshow("maze (Found Path) [dijkstra]", self.dijkstra.shortest_path_overlayed)
...
# Heap class to be used as a priority queue for Dijkstra and A*
class Heap():
    def __init__(self):
...
def main():
    """
    Main function to demonstrate path planning functionality.
    Creates a simple test graph and finds the shortest path.
    """
    print("=== Bot Path Planning Demo ===")
    
    # Create a simple test graph (example maze)
    test_graph = {
        (0, 0): {
            (0, 1): {'cost': 1},
            (1, 0): {'cost': 1},
    ...
    # Create a simple maze image (3x3 grid)
    maze = np.zeros((150, 150), dtype=np.uint8)
    # Draw grid lines
    cv2.line(maze, (50, 0), (50, 150), 255, 1)
    cv2.line(maze, (100, 0), (100, 150), 255, 1)
    cv2.line(maze, (0, 50), (150, 50), 255, 1)
    cv2.line(maze, (0, 100), (150, 100), 255, 1)
    ...
    planner.find_path_nd_display(test_graph, start, end, maze, method="a_star")
    
    print(f"Path found: {planner.path_to_goal}")
    print(f"Path length: {len(planner.path_to_goal)}")
    
    # Show the result
    cv2.imshow("Maze with Path", planner.img_shortest_path)
